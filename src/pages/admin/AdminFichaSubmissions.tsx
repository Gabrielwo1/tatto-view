...
export default function AdminFichaSubmissions() {
  ...
  async function handleDownloadPdf(sub: FichaSubmission) {
    setDownloading(true);
    try {
      await downloadFichaPdf(sub);
    } finally {
          ...
          <div className="sticky top-0 bg-zinc-950 border-b border-white/10 px-6 py-4 flex items-center justify-between gap-4 z-10">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-1.5 text-gray-400 hover:text-white transition-colors font-body text-xs font-semibold tracking-widest uppercase disabled:opacity-50"
              >
                {downloading ? (
